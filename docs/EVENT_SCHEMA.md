...
Benchmark sessions add `benchmark.evaluated` after validation completes. Its payload records the benchmark ID, hash-format version, normalized definition SHA-256, resolved starting commit, setup status/duration, overall pass/fail verdict, and one `met`, `unmet`, or `missing` result per declared expectation. A missing validation is not equivalent to an observed failure.
