...
import { ClaudeJsonAdapter, CodexJsonAdapter, ShellAdapter } from "./index.js";
...
describe("structured provider telemetry", () => {
  it("launches official non-interactive structured modes", () => {
    expect(
      new CodexJsonAdapter().launch({ passthroughArgs: ["task"] }),
    ).toEqual({
      command: "codex",
      args: ["exec", "--json", "task"],
    ...
    expect(
      new ClaudeJsonAdapter().launch({ passthroughArgs: ["task"] }),
    ).toEqual({
      command: "claude",
  ...
  it("rejects silently ignored custom-command arguments", () => {
    expect(() =>
      new ShellAdapter().launch({
        command: "node agent.js",
        passthroughArgs: ["lost argument"],
      }),
    ).toThrow(/include them in --command/);
  });
  ...
  it("records explicit provider terminal failures without response content", () => {
    const codex = new StructuredTelemetryCollector("codex");
    codex.consume(
      JSON.stringify({
        type: "turn.failed",
    ...
    expect(codex.failure()).toEqual({ source: "Codex JSONL turn.failed" });

    const claude = new StructuredTelemetryCollector("claude");
    claude.consume(
      JSON.stringify({
        type: "result",
        is_error: true,
    ...
    expect(claude.failure()).toEqual({
      source: "Claude stream-json error result",
    });
  });
  ...
  it("ignores malformed and unrelated lines", () => {
    ...
    collector.consume(
      JSON.stringify({ type: "item.completed", usage: { input_tokens: 1 } }),
    );
